...
Catalyst is currently being used within the Materials Informatics Laboratory as the graph-learning and training framework for ongoing atomistic machine-learning studies spanning structure–property prediction and the development of new graph representations. These active research workflows use the released Catalyst architecture for graph construction, model training, checkpointing, inference, and comparative model-development studies. Our prior work SODAS/SODAS++ is now integrated into Catalyst. The Materials Informatics Laboratory also routinely uses Catalyst for research in high entropy alloys, catalytic reactions, and metal-organic framework characterization. Catalyst is also the foundation for our upcoming codes focused on latent embedding exploration for atomistic trajectories and reactions as well as compositional ordering in high entropy alloys.
